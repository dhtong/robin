class Slack::AuthController < ApplicationController
  def index
    ...
    Records::CustomerUser.find_or_create_by(customer_id: customer.id, slack_user_id: res.authed_user.id)
    customer.update(slack_access_token: res.access_token, installer_customer_user_id: res.authed_user.id)
