...
RSpec.describe Commands::CreateSupportCase do
  ...
  it 'create record' do
    expect { described_class.new.execute(event_id: event.id) }.to change { Records::SupportCase.count }
  end
  ...
  context 'support case exists' do
    ...
    it 'does not create new' do
      expect { described_class.new.execute(event_id: event.id) }.not_to change { Records::SupportCase.count }
    end
  end
